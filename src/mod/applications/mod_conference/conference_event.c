...
static void send_notify(const char* profile_name,
			...
			const char* outgoing_contact_uri,
			const char* no_sub_st,
			const char* sub_st,
			const char* body) {
    ...
    switch_event_add_header_string(notify_event, SWITCH_STACK_BOTTOM, "sip_outgoing_contact_uri", outgoing_contact_uri);
    if (!zstr(no_sub_st)) {
	switch_event_add_header_string(notify_event, SWITCH_STACK_BOTTOM, "no-sub-state", no_sub_st);
    }
    if (!zstr(sub_st)) {
	switch_event_add_header_string(notify_event, SWITCH_STACK_BOTTOM, "sub-state", sub_st);
    }
...
void conference_data_rfc4579_event_handler(switch_event_t *event)
{
	...
	char *outgoing_contact_uri = switch_event_get_header(event, "sip_outgoing_contact_uri");
	char *_call_id = switch_event_get_header(event, "call-id");
	char *no_sub_st = switch_event_get_header(event, "no-sub-state");
	char *sub_st = switch_event_get_header(event, "sub-state");
	conference_obj_t *conference = NULL;
		...
		if (conference_utils_test_flag(conference, CFLAG_RFC4579)) {
		    if (refer_to_method && !strcasecmp(refer_to_method, "BYE")) {
				...
				(member = conference_member_get_by_var(conference, "caller_id_number", refer_to_user))) {
				    ...
				    send_notify(profile_name, event_name, contact_uri, from_uri, to_uri, _call_id, record_route, outgoing_contact_uri, no_sub_st, sub_st, "SIP/2.0 100 Trying\r\n");
				    ...
				    send_notify(profile_name, event_name, contact_uri, from_uri, to_uri, _call_id, record_route, outgoing_contact_uri, "", "", "SIP/2.0 200 OK\r\n");
			    }
			    else {
				...
				send_notify(profile_name, event_name, contact_uri, from_uri, to_uri, _call_id, record_route, outgoing_contact_uri, "", "", "SIP/2.0 481 Failure\r\n");
			    }
		    }
			...
			send_notify(profile_name, event_name, contact_uri, from_uri, to_uri, _call_id, record_route, outgoing_contact_uri, no_sub_st, sub_st, body);
		    }
