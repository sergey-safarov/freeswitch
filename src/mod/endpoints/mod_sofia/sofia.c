...
switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "2 sofia_handle_sip_i_refer tech_pvt->sip_call_id='%s'\n", switch_channel_get_variable(tech_pvt->channel, "sip_call_id"));
	...
	if (refer_to) {
		...
		if (rep) {
			...
			char *buf;
			char *p;
			const char *conference_name = switch_channel_get_variable(tech_pvt->channel, "conference_name");
			switch_bool_t is_conference = zstr(conference_name) ? SWITCH_FALSE : SWITCH_TRUE;
...
switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "channel sip_outgoing_contact_uri='%s'\n", switch_channel_get_variable(channel, "sip_outgoing_contact_uri"));
switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "b_channel sip_outgoing_contact_uri='%s'\n", switch_channel_get_variable(b_channel, "sip_outgoing_contact_uri"));
